...
// Affiche un récit lettre à lettre. apres() n'est appelé qu'après un
// appui une fois le texte complet — jamais tout seul.
function montrerHistoire(lignes, apres){
  ...
  zone.innerHTML = '';
  const paras = lignes.map(texte => {
    const p = document.createElement('p');
    zone.appendChild(p);
    return { p, texte, n:0 };
  });
  let i = 0, fini = false, pause = 0;
  ...
  const avancer = (e) => {
    ...
    if (!fini){ toutAfficher(); return; }   // 1ᵉʳ appui : tout le texte
    el.hidden = true;                        // 2ᵉ appui : on continue
...
// saute le récit en cours (deux appuis simulés) — utilisé par les tests
function passerHistoire(){
  if (montrerHistoire.passer){ montrerHistoire.passer(); if (montrerHistoire.passer) montrerHistoire.passer(); }
}
