...
class Combat extends Phaser.Scene {
  ...
  creerMonstre(type, x, minX, maxX){
    ...
    const m = {
      ...
      prepare:0, repos:0, vise:0, rentre:false,
    };
  ...
  creerBloc(x, contenu){
    const y = SOL_Y - 24;
    // un vrai obstacle : le même corps statique que les caisses, donc
    // les colliders déjà en place (joueur, marcheurs, objets) s'appliquent
    const corps = this.add.rectangle(x, y, 44, 44, 0xffffff, 0);
    this.physics.add.existing(corps, true);
    this.grCaisses.add(corps);
    const img = this.add.image(x, y, 'decorAtlas', 'blocPlein')
      .setScale(46 / CADRES_DECOR.blocPlein.h).setDepth(2.5);
    this.blocs.push({ img, x, y, contenu, plein:true, frappe:0, phase:this.alea()*6, corps });
  }
  majBlocs(dt){
    ...
    for (const m of this.monstres){
      ...
      if (m.def.fixe){
        ...
        if (ch && m.repos <= 0 && !m.rentre && Math.abs(dx) < ch.portee && memeNiveau){
          ...
          // En poursuite, une LAISSE : le monstre sort de sa zone jusqu'à
          // 170 px pour venir vraiment au contact. Au-delà, demi-tour —
          // il rentre au centre de sa zone en ignorant le joueur.
          let v;
          const centre = (m.minX + m.maxX) / 2;
          if (m.rentre){
            if (Math.abs(centre - m.go.x) < 40){ m.rentre = false; v = 0; }
            else v = Math.sign(centre - m.go.x) * m.def.patrouille * 1.4 * this.D.vitesse;
          } else if (Math.abs(dx) < 340 && memeNiveau){
            v = Math.sign(dx) * m.def.vitesse * this.D.vitesse;
            if (m.go.x < m.minX - 170 || m.go.x > m.maxX + 170) m.rentre = true;
          } else {
